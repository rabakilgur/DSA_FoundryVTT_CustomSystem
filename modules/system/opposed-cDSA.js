...
import DicecDSA from "./dice-cDSA.js";

export default class OpposedcDSA {
	static async handleOpposedTarget(message) {
		...
		if (actor.data.flags.oppose) {
			...
			let attacker = {
				...
				img: cDSA_Utility.getSpeaker(actor.data.flags.oppose.speaker).data.img
			};
			...
			if (testResult.successLevel > 0) {
				...
				game.user.targets.forEach(async target => {
					...
					if (attackOfOpportunity) {
						OpposedcDSA.resolveUnopposed(startMessage, game.i18n.localize("OPPOSED.attackOfOpportunity"))
					}
				})
	...
	static rerenderMessagesWithModifiers(opposeResult, attacker, defender) {
		if (opposeResult.modifiers.didModifyAttacker || attacker.testResult.modifiers) {
			...
			if (!opposeResult.swapped)
				DicecDSA.renderRollCard(chatOptions, opposeResult.attackerTestResult, attackerMessage)
			else
				DicecDSA.renderRollCard(chatOptions, opposeResult.defenderTestResult, attackerMessage)
		...
		if (opposeResult.modifiers.didModifyDefender || defender.testResult.modifiers) {
			...
			if (!opposeResult.swapped)
				DicecDSA.renderRollCard(chatOptions, opposeResult.defenderTestResult, defenderMessage)
			else
				DicecDSA.renderRollCard(chatOptions, opposeResult.attackerTestResult, defenderMessage)
		}
	}
