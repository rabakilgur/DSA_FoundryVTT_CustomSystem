...
export default class ItemRulescDSA {
	static getTalentBonus(actor, talent, types) {
		let modifier = []
		let selected = game.settings.get("cDSA", "talentModifierEnabled")
		for (let k of actor.items.filter(x => { return types.includes(x.type) && x.data.effect.value.includes(talent) })) {
			for (let m of k.data.effect.value.split(";")) {
				if (m.includes(talent)) {
					let parsed = cDSA_Utility.parseAbilityString(m.trim())
	...
	static reverseAdoptionCalculation(actor, parsed, item) {
		let adoption
		if (cDSA.vantagesNeedingAdaption[parsed.name])
			adoption = actor.items.find(x => cDSA.vantagesNeedingAdaption[parsed.name].items.includes(x.type) && x.name == parsed.special)
		else if (cDSA.AbilitiesNeedingAdaption[parsed.name])
			adoption = actor.items.find(x => cDSA.AbilitiesNeedingAdaption[parsed.name].items.includes(x.type) && x.name == parsed.special)
