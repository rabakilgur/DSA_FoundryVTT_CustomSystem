...
export default class ActorcDSA extends Actor {
	static async create(data, options) {
		...
		let skills = await cDSA_Utility.allSkills() || [];
		let combatskills = await cDSA_Utility.allCombatSkills() || [];
		let moneyItems = await cDSA_Utility.allMoneyItems() || [];
	...
	prepareDerivedData() {
		...
		try {
			...
			if (data.canAdvance) {
				...
				data.data.details.experience.description = cDSA_Utility.experienceDescription(data.data.details.experience.total)
			}
			...
			encumbrance = encumbrance ? Number(encumbrance.flags.cDSA.value) : 0
			...
			data.data.status.dodge.max = Number(data.data.status.dodge.value) + Number(data.data.status.dodge.modifier) + (Number(game.settings.get("cDSA", "higherDefense")) / 2)
			...
			if (activeGM && game.user.id == activeGM.id) {
				...
				if (pain < 4)
					pain -= AdvantageRulescDSA.vantageStep(this, game.i18n.localize('LocalizedIDs.ruggedFighter'))

				if (pain > 0)
					pain += AdvantageRulescDSA.vantageStep(this, game.i18n.localize('LocalizedIDs.sensitiveToPain'))

				pain = Math.max(Math.min(4, pain), 0)

				if (AdvantageRulescDSA.hasVantage(this, game.i18n.localize('LocalizedIDs.blind')))
					this.addCondition("blind")
				if (AdvantageRulescDSA.hasVantage(this, game.i18n.localize('LocalizedIDs.mute')))
					this.addCondition("mute")
				if (AdvantageRulescDSA.hasVantage(this, game.i18n.localize('LocalizedIDs.deaf')))
			...
			if (paralysis)
				data.data.status.speed.max = Math.round(data.data.status.speed.max * (1 - paralysis.flags.cDSA.value * 0.25))
	...
	prepareBaseData() {
		...
		for (let ch of Object.values(data.data.characteristics)) {
			...
			ch.cost = game.i18n.format("advancementCost", { cost: cDSA_Utility._calculateAdvCost(ch.initial + ch.advances, "E") })
			ch.refund = game.i18n.format("refundCost", { cost: cDSA_Utility._calculateAdvCost(ch.initial + ch.advances, "E", 0) })
		}
	...
	prepare() {
		...
		if (preparedData.canAdvance) {
			...
			for (const k of attrs) {
				preparedData.data.status[k].cost = game.i18n.format("advancementCost", { cost: cDSA_Utility._calculateAdvCost(preparedData.data.status[k].advances, "D") })
				preparedData.data.status[k].refund = game.i18n.format("refundCost", { cost: cDSA_Utility._calculateAdvCost(preparedData.data.status[k].advances, "D", 0) })
			}
		}
	...
	static _calculateCombatSkillValues(i, actorData) {
		...
		i.cost = game.i18n.format("advancementCost", { cost: cDSA_Utility._calculateAdvCost(i.data.talentValue.value, i.data.StF.value) })
		i.canAdvance = ActorcDSA.canAdvance(actorData)
		return i;
	}

	_perpareItemAdvancementCost(item) {
		item.cost = game.i18n.format("advancementCost", { cost: cDSA_Utility._calculateAdvCost(item.data.talentValue.value, item.data.StF.value) })
		item.refund = game.i18n.format("refundCost", { cost: cDSA_Utility._calculateAdvCost(item.data.talentValue.value, item.data.StF.value, 0) })
		item.canAdvance = ActorcDSA.canAdvance(this.data)
		return item
	}
	...
	prepareItems() {
		...
		for (let t in cDSA.equipmentTypes) {
			inventory[t] = {
				items: [],
				show: false,
				dataType: t
				...
				switch (i.type) {
					...
					case "trait":
						switch (i.data.traitType.value) {
							case "rangeAttack":
								i = ActorcDSA._prepareRangeTrait(i)
								break
							case "meleeAttack":
								i = ActorcDSA._prepareMeleetrait(i)
								break
							case "armor":
						...
						break
					case "combatskill":
						combatskills.push(ActorcDSA._calculateCombatSkillValues(i, actorData));
						break;
					case "ammunition":
					...
					case "meleeweapon":
						...
						inventory.meleeweapons.items.push(ActorcDSA._prepareitemStructure(i));
					...
					case "rangeweapon":
						i.weight = parseFloat((i.data.weight.value * i.data.quantity.value).toFixed(3));
						i.toggleValue = i.data.worn.value || false;
						inventory.rangeweapons.items.push(ActorcDSA._prepareitemStructure(i));
						inventory.rangeweapons.show = true;
						totalWeight += Number(i.weight);
						break;
					case "armor":
						i.toggleValue = i.data.worn.value || false;
						inventory.armor.items.push(ActorcDSA._prepareitemStructure(i));
					...
					case "consumable":
						i.weight = parseFloat((i.data.weight.value * i.data.quantity.value).toFixed(3));
						inventory[i.data.equipmentType.value].items.push(ActorcDSA._prepareConsumable(i));
					...
					case "equipment":
						i.weight = parseFloat((i.data.weight.value * i.data.quantity.value).toFixed(3));
						inventory[i.data.equipmentType.value].items.push(ActorcDSA._prepareitemStructure(i));
						inventory[i.data.equipmentType.value].show = true;
						totalWeight += Number(i.weight);
						break;
					case "money":
		...
		for (let wep of inventory.rangeweapons.items) {
			if (wep.data.worn.value)
				rangeweapons.push(ActorcDSA._prepareRangeWeapon(wep, inventory.ammunition.items, combatskills, this));
		}
		...
		for (let wep of wornweapons) {
			meleeweapons.push(ActorcDSA._prepareMeleeWeapon(wep, combatskills, actorData, wornweapons.filter(x => x._id != wep._id && !regex2h.test(x.name))))
		}
		...
		encumbrance = Math.max(0, encumbrance - SpecialabilityRulescDSA.abilityStep(this.data, game.i18n.localize('LocalizedIDs.inuredToEncumbrance')))
		...
		let characteristics = duplicate(cDSA.characteristics)
		characteristics["-"] = "-"
	...
	_applyModiferTransformations(itemModifiers) {
		for (const [key, value] of Object.entries(itemModifiers)) {
			let shortCut = game.cDSA.config.knownShortcuts[key.toLowerCase()]
			if (shortCut)
	...
	async _updateAPs(APValue) {
		if (ActorcDSA.canAdvance(this.data)) {
	...
	async checkEnoughXP(cost) {
		...
		if (Number(this.data.data.details.experience.total) - Number(this.data.data.details.experience.spent) >= cost) {
			...
			let selOptions = Object.entries(cDSA.startXP).map(([key, val]) => `<option value="${key}">${game.i18n.localize(val)} (${key})</option>`).join("")
	...
	setupWeaponTrait(item, mode, options) {
		options["mode"] = mode
		return ItemcDSA.getSubClass(item.type).setupDialog(null, options, item, this)
	}
	...
	setupWeaponless(statusId, options = {}) {
		let item = duplicate(cDSA.defaultWeapon)
	...
	setupSpell(spell, options = {}) {
		return ItemcDSA.getSubClass(spell.type).setupDialog(null, options, spell, this)
	}

	setupSkill(skill, options = {}) {
		return ItemcDSA.getSubClass(skill.type).setupDialog(null, options, skill, this)
	}
		...
		if (this.data.data.status.fatePoints.value > 0) {
			...
			switch (type) {
				case "rerollDamage":
					...
					DicecDSA.showDiceSoNice(newRoll, newTestData.rollMode)
	...
	setupRegeneration(statusId, options = {}) {
		...
		let dialogOptions = {
			...
			data: {
				...
				regnerationCampLocations: cDSA.regnerationCampLocations
			},
			callback: (html) => {
	...
	setupCharacteristic(characteristicId, options = {}) {
		...
		let dialogOptions = {
			...
			callback: (html) => {
				cardOptions.rollMode = html.find('[name="rollMode"]').val();
				testData.testModifier = Number(html.find('[name="testModifier"]').val());
				testData.testDifficulty = cDSA.attributeDifficultyModifiers[html.find('[name="testDifficulty"]').val()];
				testData.situationalModifiers = ActorcDSA._parseModifiers('[name="situationalModifiers"]')
				return { testData, cardOptions };
			}
		};
	...
	static _prepareRangeWeapon(item, ammunitions, combatskills, actor) {
		...
		if (skill) {
			...
			item.LZ = Math.max(0, Number(item.data.reloadTime.value) - SpecialabilityRulescDSA.abilityStep(actor, `${game.i18n.localize('LocalizedIDs.quickload')} (${game.i18n.localize(item.data.combatskill.value)})`))
		} else {
			ui.notifications.error(game.i18n.format("DSAError.unknownCombatSkill", { skill: item.data.combatskill.value, item: item.name }))
		}
		return this._parseDmg(item)
	...
	async addCondition(effect, value = 1, absolute = false, auto = true) {
		...
		if (existing && existing.flags.cDSA.value == null)
			return existing
		else if (existing) {
			return await cDSAStatusEffects.updateEffect(this, existing, value, absolute, auto)
		} else if (!existing) {
			return await cDSAStatusEffects.createEffect(this, effect, value, auto)
		}
	}

	async _dependentEffects(statusId, effect, delta) {
		if (effect.flags.cDSA.value == 4 && ["encumbered", "stunned", "feared", "inpain", "confused"].includes(statusId))
			await this.addCondition("incapacitated")

		if (effect.flags.cDSA.value == 4 && (statusId == "paralysed"))
			await this.addCondition("rooted")

		if (statusId == "unconscious")
			await this.addCondition("prone")

		if (delta > 0 && statusId == "inpain" && !this.hasCondition("bloodrush") && AdvantageRulescDSA.hasVantage(this, game.i18n.localize('LocalizedIDs.frenzy'))) {
	...
	async removeCondition(effect, value = 1, auto = true, absolute = false) {
		...
		if (existing && existing.flags.cDSA.value == null) {
			...
			return await cDSAStatusEffects.removeEffect(this, existing, value, absolute, auto)
		}
	}
